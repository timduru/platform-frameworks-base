...
public class TabletStatusBarView extends FrameLayout {
    ...
    public void setBar(BaseStatusBar phoneStatusBar) {
        ...
        ArrayList<View> children = ((TabletStatusBar) phoneStatusBar).getEos().getAllChildren(this);
        for (View child : children) {
            if (child instanceof FeatureListener) {
                ((TabletStatusBar) phoneStatusBar).getEosObserver().registerClass(
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        // Find the view we wish to grab events from in order to detect search
        // gesture.
        // Depending on the device, this will be one of the id's listed below.
        // If we don't find one, we'll use the view provided in the constructor
        // above (this view).
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            // do not close the recents panel here- the intended behavior is
            // that recents is dismissed
            // on touch up when clicking on status bar buttons
            // TODO: should we be closing the notification panel and input
            // methods panel?
    ...
    /**
     * Let the status bar know that if you tap on ignore while panel is showing,
     * don't do anything. Debounces taps on, say, a popup's trigger when the
