...
public class EosGlassController {

    private static Glass mGlass = null;

    public static void startGlass(Context context,
            PhoneStatusBarView statbar, StatusBarWindowView window) {
        mGlass = new Glass(context, statbar, window);
    }

    public static void setNavigationBar(NavigationBarView navbar) {
        if (mGlass != null) {
            mGlass.setNavigationBarView(navbar);
        }
    }
    ...
    public static boolean isGlassEnabled() {
        if (mGlass == null)
            return false;
        return mGlass.mGlassEnabled;
    }

    public static void applyGlassEffect() {
        if (mGlass != null) {
            mGlass.applyGlassEffect();
        }
    }

    public static void updateBarColors() {
        if (mGlass != null) {
            mGlass.updateBarColors();
        }
    }

    private static class Glass implements OnFeatureStateChangedListener {
        ...
        public Glass(Context context, PhoneStatusBarView statbar,
        ...
        private void setNavigationBarView(NavigationBarView navbar) {
            mNavigationBarView = navbar;
            handleGlassChange();
        }

        private void registerUriList() {
            MSG_STATUSBAR_COLOR_SETTINGS = EosObserverHandler.getEosObserverHandler()
                    .registerUri(EOSConstants.SYSTEMUI_STATUSBAR_COLOR);
            MSG_NAVBAR_COLOR_SETTINGS = EosObserverHandler.getEosObserverHandler()
                    .registerUri(EOSConstants.SYSTEMUI_NAVBAR_COLOR);
            MSG_GLASS_SETTINGS = EosObserverHandler.getEosObserverHandler()
                    .registerUri(EOSConstants.SYSTEMUI_USE_GLASS);
            MSG_GLASS_NAVBAR_SETTINGS = EosObserverHandler.getEosObserverHandler()
                    .registerUri(EOSConstants.SYSTEMUI_NAVBAR_GLASS_LEVEL);
            MSG_GLASS_STATUSBAR_SETTINGS = EosObserverHandler.getEosObserverHandler()
                    .registerUri(EOSConstants.SYSTEMUI_STATUSBAR_GLASS_LEVEL);
            MSG_GLASS_NAVBAR_PRESET = EosObserverHandler.getEosObserverHandler()
                    .registerUri(EOSConstants.SYSTEMUI_NAVBAR_GLASS_DEFAULT_ENABLED);
            MSG_GLASS_STATUSBAR_PRESET = EosObserverHandler.getEosObserverHandler()
                    .registerUri(EOSConstants.SYSTEMUI_STATUSBAR_GLASS_DEFAULT_ENABLED);

            EosObserverHandler.getEosObserverHandler().setOnFeatureStateChangedListener(
                    (OnFeatureStateChangedListener) Glass.this);
        }
