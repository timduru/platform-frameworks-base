...
public class BatteryTile extends QuickSettingsTile implements BatteryStateChangeCallback{
    ...
    @Override
    public void onBatteryLevelChanged(int level, boolean status) {
        mBatteryLevel = level;
    ...
    private synchronized void updateTile() {
        final int drawableResId = mBatteryStatus ? R.drawable.qs_sys_battery_charging : R.drawable.qs_sys_battery;

        mBatteryIcon = mContext.getResources().getDrawable(drawableResId);

        if (mBatteryStatus) mLabel = mContext.getString(R.string.quick_settings_battery_charging_label, mBatteryLevel);
        else mLabel = mContext.getString(R.string.status_bar_settings_battery_meter_format, mBatteryLevel);
    }
